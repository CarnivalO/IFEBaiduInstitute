...
function renderChart() {
	...
	for (var v in chartData) {
		...
		str += "<div class='historygram' style='height:" + chartData[v] + "px;background-color:" + getRandomColor() + "' title='" + v + ":" + chartData[v] + "'></div>";
		str += "</div>";
	};
	var dateobj = {
		day: '日',
	...
	document.getElementsByClassName("aqi-chart-wrap")[0].innerHTML = '<legend>' + pageState["nowSelectCity"] + '空气质量</legend>' + str;
}
...
function getTimeNow() {
	...
	var typeNow = "";
	[].forEach.call(types, function(dateType) {
		if (dateType.checked) {
			dateType.previousSibling.setAttribute('class', 'selected');
			typeNow = dateType.value;
		} else {
			dateType.previousSibling.setAttribute('class', '');
		}
	});
	return typeNow;
}
